...
FIGURE = dict(left=.055, bottom=.145, width=.705, height=.70, inches_width=16, inches_height=11)
...
def load_forecast(config_path):
    ...
    return dict(run=run, frames=frames, locations=list(locations),
...
body{margin:0;padding:20px}.shell{max-width:1180px;margin:auto}.heading{display:flex;justify-content:space-between;gap:16px;align-items:end;flex-wrap:wrap}
h1{font-size:25px;margin:0 0 4px}.meta{color:#607480}.map{position:relative;margin-top:14px}.map>img{display:block;width:100%;height:auto}.coast-overlay{position:absolute;inset:0;width:100%;height:100%;pointer-events:none}.coast-segment{fill:none;stroke-linecap:round;stroke-width:4px;vector-effect:non-scaling-stroke}
.marker{position:absolute;transform:translate(-50%,-50%);width:76px;height:104px;border:0;background:transparent;color:#173440;cursor:pointer;padding:0}
.arrow{position:absolute;left:calc(50% - 27px);top:calc(50% - 27px);width:54px;height:54px;filter:drop-shadow(0 1px 2px #fff);transform-origin:27px 27px}.arrow:before{content:"";position:absolute;left:24px;top:7px;width:6px;height:42px;border-radius:3px;background:currentColor}.arrow:after{content:"";position:absolute;left:15px;top:4px;width:18px;height:18px;border-left:6px solid currentColor;border-top:6px solid currentColor;transform:rotate(45deg)}.value{position:absolute;left:50%;top:78%;transform:translateX(-50%);display:inline-block;background:rgba(247,249,250,.9);border-radius:7px;padding:2px 5px;font-weight:700}
...
@media(max-width:600px){body{padding:8px}.marker{width:52px;height:72px}.arrow{left:calc(50% - 18px);top:calc(50% - 18px);width:36px;height:36px;transform-origin:18px 18px}.arrow:before{left:16px;top:5px;width:4px;height:28px}.arrow:after{left:10px;top:3px;width:12px;height:12px;border-width:4px}.value{font-size:11px;top:76%}.controls{grid-template-columns:auto 1fr}.controls select{grid-column:1/-1}.chart{height:185px}.direction-chart{height:155px}}
</style></head><body><main class="shell"><div class="heading"><div><h1>Vento sull’Isola d’Elba</h1><div class="meta" id="time"></div></div><div class="meta"><div>Freccia: verso del moto · numero: km/h</div><div class="coast-key"><span>Costa non esposta</span><span class="onshore">Onshore nelle ultime 6 ore</span></div></div></div>
...
<div class="legend"><span class="key">Velocità (km/h)</span><span class="key gust">Raffica (km/h)</span><span class="key predictability">Prevedibilità ensemble (%)</span><span class="key direction">Verso e componente nord/sud (km/h)</span></div>
