...
<canvas class="chart direction-chart" id="directionChart" role="img" aria-label="Direzione del vento ora per ora: scala angolare lineare nord sud, est e ovest zero, diagonali piu o meno 0,5, nord piu uno e sud meno uno"></canvas>
...
function drawDirection(){const {c,w,h}=setupCanvas(directionCanvas),series=forecast.frames.map(f=>f.values.find(v=>v.id===selected)),components=series.map(v=>v.direction==null?0:1-Math.abs(((v.direction%360)+360)%360-180)/90),limit=1,pad={l:58,r:12,t:12,b:48},x=i=>pad.l+i*(w-pad.l-pad.r)/(series.length-1),y=v=>pad.t+(limit-v)*(h-pad.t-pad.b)/(2*limit);c.strokeStyle='#d5e0e5';c.lineWidth=1;[-limit,-0.5,0,0.5,limit].forEach(v=>{const yy=y(v);c.beginPath();c.moveTo(pad.l,yy);c.lineTo(w-pad.r,yy);c.stroke();c.fillStyle='#607480';c.font='11px sans-serif';c.fillText(v===0?'E/O 0':(v>0?'N +':'S −')+Math.abs(v),3,yy+4)});timeAxis(c,w,h,pad,x);c.strokeStyle='#7b2cbf';c.fillStyle='#7b2cbf';c.lineWidth=1.5;series.forEach((v,i)=>{if(v.direction==null)return;const flow=(v.direction+180)*Math.PI/180,cx=x(i),cy=y(components[i]),dx=Math.sin(flow)*5,dy=-Math.cos(flow)*5;c.beginPath();c.moveTo(cx-dx,cy-dy);c.lineTo(cx+dx,cy+dy);c.stroke();const ex=cx+dx,ey=cy+dy,px=-dy*.55,py=dx*.55;c.beginPath();c.moveTo(ex,ey);c.lineTo(ex-dx*.8+px,ey-dy*.8+py);c.lineTo(ex-dx*.8-px,ey-dy*.8-py);c.closePath();c.fill()});selectedGuide(c,h,pad,x)}
